...
typedef enum php_pcov_wants_status {
	WANTS_UNKNOWN = 0,
	WANTS_YES     = 1,
	WANTS_NO      = 2,
} php_pcov_wants_status;

typedef struct php_pcov_op_array_info_data {
	php_pcov_wants_status wants;
} php_pcov_op_array_info_data;
...
static zend_always_inline php_pcov_op_array_info_data *php_pcov_get_op_array_info_ptr(const zend_op_array *op_array) {
	ZEND_ASSERT(op_array->type == ZEND_USER_FUNCTION);
#if PHP_PCOV_USE_RID_CACHE
	if (EXPECTED(!(op_array->fn_flags & (ZEND_ACC_CALL_VIA_TRAMPOLINE | ZEND_ACC_ABSTRACT)))) {
		return (php_pcov_op_array_info_data *) &PHP_PCOV_OP_ARRAY_INFO(op_array);
	} else {
		return NULL;
	}
#else
...
static zend_always_inline zend_bool php_pcov_wants_at_runtime(const zend_op_array *op_array) {
	php_pcov_op_array_info_data *data = php_pcov_get_op_array_info_ptr(op_array);
	if (data) {
		if (data->wants == WANTS_UNKNOWN) {
			data->wants = php_pcov_wants(op_array->filename) ? WANTS_YES : WANTS_NO;
		}
		return data->wants == WANTS_YES;
	}
